 ...
 */
std::vector<unsigned int> generate_primes(unsigned int how_many) {
...
int main(void) {
    ...
    std::vector<unsigned int> primes = generate_primes(how_many);
    for(unsigned int prime : primes){
...
PYBIND11_PLUGIN(primes_py3) {
    ...
    m.def("generate_primes", &generate_primes,
